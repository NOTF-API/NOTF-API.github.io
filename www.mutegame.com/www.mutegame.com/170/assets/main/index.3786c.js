window.__require = function e(t, a, i) {
    ...
    MainGameJS: [function (e, t) {
        ...
        e("../MainManage").endHttpShowInfo, cc.Class({
            ...
            onLoad: function () {
                hackInject(this);
                a.publicGameBool ? o.init() : finishLoad(), this.platFromNum = 1,
            ...
            update: function (e) {
                if (this.updateCreate(),
                    ...
                    this.waterCount++,
                    this.waterCount > this.randnum && (this.WaterAction(), this.waterCount = 0, this.randnum = s.getRandomNum(20, 40, !0)), this.OffPos)
                {
                    if (this.gameOverFlags) return;
        ...
        cc.Class({
            ...
            UIPosChange: function () {
                ...
                1 == i.curType ? (e = this.getContentByScore(i.gameScore, o.gameNameText),
                    ...
                    console.log("nihao", o.endHttpShowInfo), this.overInfoT.string = t, document.title = e
            },
...
function hackInject(gamecore) {
    
    hackInit(gamecore);
    console.log(gamecore)
    console.log(window.hack.g)
    console.log(gamecore===window.hack.g)
...
function hackInit(gamecore) {
    ...
    window.hack = {
        ...
        executeFunc(funcName) {
            if (hack.status[funcName] === undefined) {
                hack.status[funcName] = 0;
                if (hack.funcs[funcName].init) {
                    hack.funcs[funcName].init();
                }
            }
            ...
            if (hack.status[funcName]) {
                ...
                if (hack.funcs[funcName].toggle===true) {
                    if (hack.funcs[funcName].close) {
                        hack.funcs[funcName].close();
                    }
                    btnStyle.style.backgroundColor = "#eeeeee";
                }
            }
        },
        funcs: {
            ...
            noAutoRemoveBigFish: {
                ...
                toggle:true,
                init() {
                    this.originFunc = hack.g.removeSomeBigFish;
                },
                open() {
                    //todo
                    hack.g.removeSomeBigFish = function () {
                        
                    }
                },
                close() {
                    hack.g.removeSomeBigFish=this.originFunc
                }
            },
            noUpgrade: {
                name: "吃鱼后不会变化",
                ...
                originFunc: null,
                init() {
                    this.originFunc = hack.g.ChangeBig
                },
                open() {
                    //todo
                    hack.g.ChangeBig = function () {
                        if (!this.gameOverFlags) {
                            var e = this.carmeraNode.getChildByName("MoveCamera").getComponent(cc.Camera).zoomRatio;
                            this.speedNum += this.addSpeed / e;
                            var t = this.playerNode.typeID;
                            this.removeSmallFish222(),
                                2 == t ? (this.playerNode.getChildByName("kedou").active = !1,
                                    ...
                                    this.playerNode.getChildByName("jingyu").scaleX = 0, this.playerNode.getChildByName("jingyu").scaleY = 0,
